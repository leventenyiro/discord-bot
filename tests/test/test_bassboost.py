...
class TestBassboostCommand(unittest.TestCase):
    error_messages = {
        'no_server': 'I am not on any voice channel!',
        'member_not_on_voice': 'You have to be in a voice channel to toggle the bassboost mode!',
    ...
    def test_bassboost_basic(self):
        ...
        command = BassboostCommand(ctx, self.music_bot)
        response = asyncio.run(command.run(logging=False))
        self.assertIsNone(response)
        self.assertTrue(audio_player.is_bassboost_mode())
        command = BassboostCommand(ctx, self.music_bot)
        response = asyncio.run(command.run(logging=False))
        self.assertIsNone(response)
        self.assertFalse(audio_player.is_bassboost_mode())

    def test_bassboost_no_server(self):
        ctx = MockContext()
        command = BassboostCommand(ctx, self.music_bot)
        result = asyncio.run(command.run(logging=False))
        self.assertEqual(result, self.error_messages['no_server'])

    def test_bassboost_member_not_on_voice(self):
    ...
    def test_bassboost_not_the_same_text_channel(self):
    ...
    def test_bassboost_not_the_same_voice_channel(self):
        voice_channel = MockVoiceChannel(id=69, name='asd')
        audio_player = AudioPlayer(MockVoiceClient())
        server = {'voice_channel': voice_channel, 'text_channel': text_channel, 'audio_player': audio_player}
        audio_player.add_to_playlist(unittest.mock.MagicMock())
