...
class PauseCommand(BaseCommand):
    def __init__(self, ctx, music_bot) -> None:
        ...
        required_permissions = [
            ...
            (ctx.author.voice is not None, InfoEmbed('You have to be in a voice channel to pause the current song!')),
        ...
        response = [
            InfoEmbed(f'{"Resuming" if self.server and self.server["audio_player"].is_paused() else "Pausing"}!')
        ]
        super().__init__(ctx, required_permissions, response)

    async def logic(self, logging=True):
        if logging:
            Logger.info(f'{"Pausing" if self.server and self.server["audio_player"].is_paused() else "Resuming"} on {self.ctx.channel.guild.id}')
