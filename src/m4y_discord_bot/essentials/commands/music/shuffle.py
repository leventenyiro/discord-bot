...
class ShuffleCommand(BaseCommand):
    def __init__(self, ctx, music_bot) -> None:
        ...
        response = [
            InfoEmbed(f'Shuffle mode is {"off" if self.server and self.server["audio_player"].is_shuffle() else "on"}!')
        ]
    ...
    async def logic(self, logging=True):
        ...
        if logging:
            Logger.info(f'Shuffle mode is {"on" if self.server and self.server["audio_player"].is_shuffle() else "off"} on {self.ctx.channel.guild.id}!')
