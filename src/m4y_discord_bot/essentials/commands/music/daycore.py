...
class DaycoreCommand(BaseCommand):
    def __init__(self, ctx, music_bot) -> None:
        ...
        required_permissions = [
            ...
            (ctx.author.voice is not None, InfoEmbed('You have to be in a voice channel to toggle the daycore mode!')),
        ...
        response = [
            InfoEmbed(f'Daycore mode is {"off" if self.server and self.server["audio_player"].is_daycore_mode() else "on"}!') 
        ]
        super().__init__(ctx, required_permissions, response)

    async def logic(self, logging=True):
        audioplayer = self.server['audio_player']
        audioplayer.toggle_daycore()
        if logging:
            Logger.info(f'Daycore mode is {"on" if self.server and self.server["audio_player"].is_daycore_mode() else "off"} on {self.ctx.channel.guild.id}!')
