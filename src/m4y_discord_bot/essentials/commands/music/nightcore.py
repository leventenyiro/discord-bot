...
class NightcoreCommand(BaseCommand):
    def __init__(self, ctx, music_bot) -> None:
        ...
        required_permissions = [
            ...
            (ctx.author.voice is not None, InfoEmbed('You have to be in a voice channel to toggle the nightcore mode!')),
        ...
        response = [
            InfoEmbed(f'Nightcore mode is {"off" if self.server and self.server["audio_player"].is_nightcore_mode() else "on"}!') 
        ]
        super().__init__(ctx, required_permissions, response)

    async def logic(self, logging=True):
        audioplayer = self.server['audio_player']
        audioplayer.toggle_nightcore()
        if logging:
            Logger.info(f'Nightcore mode is {"on" if self.server and self.server["audio_player"].is_nightcore_mode() else "off"} on {self.ctx.channel.guild.id}!')
