...
class ResetSpeedCommand(BaseCommand):
    ...
    def __init__(self, ctx, music_bot) -> None:
        ...
        required_permissions = [
            ...
            (ctx.author.voice is not None, InfoEmbed('You have to be in a voice channel to reset speed of the music!')),
        ...
        response = [
            InfoEmbed(f'Music speed has been reseted!')
        ]
        super().__init__(ctx, required_permissions, response)

    async def logic(self, logging=True):
        if logging:
            Logger.info(f'Music speed has been reseted on {self.ctx.channel.guild.id}')
