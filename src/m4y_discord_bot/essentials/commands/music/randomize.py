...
class RandomizeCommand(BaseCommand):
    ...
    async def logic(self, logging=True):
        ...
        if logging:
            Logger.info(f'{PromptColors.CGREENBG}{self.ctx.guild.id}{PromptColors.CEND} Playlist randomized!')
        player.randomize()
